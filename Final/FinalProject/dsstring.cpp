...
DSString::DSString(const char* rhs){
    ...
    len = cap = strlen(rhs) + 1; //length and capacity of cstring (null terminator taken into account)
    data = nullptr; //set to nullptr to ensure it doesn't point to other memory
    data = new char[cap]; //set data to array the length of capacity

    for(int i = 0; i < len; i++){
        data[i] = rhs[i]; //initialize elements of data with cstring characters
    }
}
...
DSString& DSString::operator= (const char *rhs){
    ...
    len = strlen(rhs) + 1; //length is equal to length of cstring plus null terminator
    cap = strlen(rhs) + 1;

    delete[] data; //deallocated memory data points to
    data = nullptr; //ensure data doesn't point to random memory
    data = new char[cap]; //set data to point to an array the size of cap

    for(int i = 0; i< len; i++)
        data[i] = rhs[i]; //fill data with the elements from rhs cstring
...
DSString DSString::operator+ (const DSString& rhs){
    ...
    int newcap = rhs.len + len; //create a new capacity of lengths combined
    char* temp = nullptr;
    temp = new char[newcap]; //create temporary char * array
    int elcount = 0; //initialize a counter for the elements in the temp array
    for(int i =0; i < (len - 1); i++){ //len-1 so that nullterminator for left side is left
...
int DSString::size(){
    return len - 1; //return length minus null terminator for size
}
...
std::ostream& operator<< (std::ostream& output, const DSString& str){
    for (int i= 0; i < (str.len - 1); i++) //go through each element of data & store in output
        output << str.data[i];
